...
* **Generic PostScript Printer:** PostScript-compatible printer with PDF output.
  ...
  * The original .ps files may remain in the directory if PDF conversion fails, or (on Windows hosts) if the included ``gsdll64.dll`` file is missing from the 86Box directory.
