...
.. note:: Discord integration will not be available if the Discord desktop app is not running, or (on Windows hosts) if the included ``discord_game_sdk.dll`` file is missing from the 86Box directory.
